...
const DMChatPage = () => {
  ...
  return (
    <div className="fixed inset-0 flex flex-col bg-base-200" style={{ paddingTop: 'env(safe-area-inset-top, 0)' }}>
      {/* 헤더 - 고정 */}
      <header className="flex-shrink-0 bg-base-100 border-b border-base-300 px-4 py-3 flex items-center gap-3 shadow-sm z-10">
        <button
          onClick={handleBack}
          className="p-2 -ml-2 text-base-content/60 active:bg-base-200 rounded-full"
        >
          <ArrowBackIcon />
        </button>
        ...
        <div className="flex-1 min-w-0">
          ...
          {otherUser?.username && otherUser?.name !== otherUser?.username && (
            <p className="text-sm text-base-content/50 truncate">@{otherUser.username}</p>
          )}
        </div>
      </header>
      ...
      <main className="flex-1 overflow-y-auto px-4 py-3 overscroll-contain">
        {isLoading ? (
          <div className="flex justify-center items-center h-full">
            <div className="text-base-content/50">메시지를 불러오는 중...</div>
          </div>
        ) : messages.length === 0 ? (
            ...
            {Object.entries(messageGroups).map(([date, msgs]) => (
              <React.Fragment key={date}>
                ...
                <div className="flex items-center my-4">
                  ...
                  <span className="px-3 text-sm text-base-content/50 font-medium">
                  ...
                  return (
                    ...
                    >
                      <div className="max-w-[75%]">
                        ...
                        <p className={`text-xs text-base-content/40 mt-1 ${isMyMessage ? 'text-right' : 'text-left'}`}>
      ...
      {/* 입력창 - 고정 */}
      <footer className="flex-shrink-0 bg-base-100 border-t border-base-300 p-3" style={{ paddingBottom: 'calc(12px + env(safe-area-inset-bottom, 0))' }}>
        <form onSubmit={handleSend} className="flex items-center gap-2">
          <input
            ...
            className="flex-1 px-4 py-3 bg-base-200 rounded-full text-base outline-none focus:ring-2 focus:ring-green-400 focus:bg-base-100"
            style={{ fontSize: '16px' }}
          />
          <button
            type="submit"
            disabled={!messageText.trim() || sendMessageMutation.isPending}
            className="p-3 bg-green-500 text-white rounded-full active:bg-green-600 disabled:bg-base-content/20 disabled:cursor-not-allowed"
          >
            <SendIcon />
          </button>
        </form>
